...
file_path = "..\\dl_models\\shape_fraction_model\\labels_dict.json"
SHAPE_MODEL_LABELS_FILE = open(file_path)
SHAPE_MODEL_LABELS = json.load(SHAPE_MODEL_LABELS_FILE)


def load_math_model():
    return keras.models.load_model(MATH_MODEL_PATH)


def load_shape_model():
    return keras.models.load_model(SHAPE_MODEL_PATH)
...
def preprocess_shape_image(img):
    ...
    # Add additional dimension to the image array
...
def detect_shape(segmented_images):
    try:
        shape_model = load_shape_model()
        fraction_model = load_math_model()
        ...
        denominator_img = fraction_image.crop(
            (0, fraction_image.height / 2, fraction_image.width, fraction_image.height))
        numerator_img = numerator_img.resize((28, 28))
        denominator_img = denominator_img.resize((28, 28))
        numerator_img = np.asarray(numerator_img)
        denominator_img = np.asarray(denominator_img)
        numerator_img = np.reshape(numerator_img, (1, 28, 28, 1))
        denominator_img = np.reshape(denominator_img, (1, 28, 28, 1))
        fraction_prediction = labels[np.argmax(fraction_model.predict(numerator_img))]
        fraction_prediction += "/"
        fraction_prediction += labels[np.argmax(fraction_model.predict(denominator_img))]
...
def detect_equation(segmented_images):
    ...
    knn_model = joblib.load(open(FRACTION_MODEL_PATH, 'rb'))

    for i in segmented_images:
        fraction_image = resize_image(i)
        fraction_image = get_hog_image(fraction_image)
        ...
        if FRACTION_MODEL_LABELS[predict[0]] != "fraction":
            ...
            denominator_img = i.crop((0, i.height / 2, i.width, i.height))
            numerator_img = numerator_img.resize((28, 28))
            denominator_img = denominator_img.resize((28, 28))
            numerator_img = np.asarray(numerator_img)
            denominator_img = np.asarray(denominator_img)
            ...
            pred_denominator = labels[np.argmax(model.predict(denominator_img))]
            if "0" in pred_numerator and "0" in pred_denominator:
                equation += "8"
            elif "0" in pred_denominator:
                ...
                equation += "/"
                equation += labels[np.argmax(model.predict(denominator_img))]
                equation += ")"
...
def detect(segmented_equation_path, is_shape):
    ...
    if is_shape:
        return detect_shape(segmented_images)
    else:
